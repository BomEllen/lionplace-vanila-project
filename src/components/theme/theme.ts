...
class Theme extends LitElement {
  ...
  private handleMoveButtonClick(e: Event) {
      ...
      <div class="theme-wrap">
        ${this.test.map(
          (item: Item, itemIndex: number) => html`
            <div @click=${this.handleMoveButtonClick} class="sub-image" style="background-image: linear-gradient(to bottom, rgba(255, 255, 255, 0), rgba(255, 255, 255, 50)), url('${this.backgroundImageUrl}');">
              <button disabled>임시저장</button>
              <!-- 버튼 클릭 비활성화 -->
              <div class="text-box">
                <h2>${item.name}</h2>
        ...
        <!-- 플러스 버튼 기능 영역 -->
        <div class="theme-wrap" @click=${this.handleAddButtonClick}>
          <div class="sub-image">
