  ...
  async fetchData() {
    ...
    try {
      const visitRecords = await pb
        ...
        .getFullList({ expand: "place,review.tags", sort: "+date" });
      ...
      this.visitRecords = visitRecords.map((item): VisitData => {
        if (item.review === "") {
          ...
          const tags = item.expand!.review.expand!.tags.map(
            (i: { text: string }) => i.text
          );
          return {
            price: item.expand!.place.placeName,
            date: this.formatToDate(item.created),
            placeName: item.expand!.place.placeName,
            reviewText: item.expand!.review.text,
            reviewImg: getImageURL(
              item.collectionId,
              item.id,
              item.expand!.review.img
            ),
            reviewTags: tags,
          };
        }
    ...
    return html`
      <div class="review-visit-record-wrap">
        ${this.visitRecords?.map((item) => {
          if (item.reviewText !== "") {
            return html`<visit-record-complete
              .data=${item}
            ></visit-record-complete>`;
          } else {
            return html`<visit-record-noreview
              .data=${item}
            ></visit-record-noreview>`;
          }
        })}
      </div>
    `;
